...
function CommissionTopicDetails() {
    ...
    useEffect(() => {
        const resolveTermId = async () => {
            ...
            if (cachedSession?.municipalityMandateId) {
                setTermId(cachedSession.municipalityMandateId);
                return;
            }

            try {
                const { data } = await api.get(`/api/sessions/${sessionId}`);
                setTermId(data.municipalityMandateId);
            } catch (error) {
                console.error('Error resolving session term:', error);
            }
        };

        if (sessionId) resolveTermId();
    }, [municipalityId, sessionId]);

    // Populate the user-image cache once per term (mirrors Topics.js)
    useEffect(() => {
        if (!termId) return;
        if (isTermPopulated(termId)) return;
    ...
    useEffect(() => {
        const fetchDetails = async () => {
            ...
            try {
                const { data } = await api.get(`/api/commission-topics/${commissionTopicId}/details`);
                ...
                console.error("Failed to fetch commission topic details:", error);
            } finally {
                setLoading(false);
            }
        };

        if (commissionTopicId) {
            fetchDetails();
        }
    }, [commissionTopicId]);
         ...
         <div  className="table-wrapper">
            <table className="details-table">
                ...
                {[...(topicDetails.yesUsers || [])].map((user, index) => (
                        <tr key={`yes-${index}`}>
                            <td>
                                <UserAvatar username={user.username} name={user.name} surname={user.surname} className="details-image" termId={termId} />
                            </td>
...
export default CommissionTopicDetails;
