...
const LiveUsersModal = ({ isOpen, onClose, municipalityId, token, role, status, canSeeAction  }) => {
  ...
  const fetchOnlineUsers = useCallback(async () => {
    try {
      const response = await api.get(`/api/municipalities/${municipalityId}/online-users-list`);
  ...
  const fetchOfflineUsers = useCallback(async () => {
    try {
      const response = await api.get(`/api/municipalities/${municipalityId}/offline-users-list`);
  ...
  const makeUserOffline = async (username) => {
    try {
      await api.get(`/api/municipalities/${municipalityId}/offline/${username}`);
      fetchOnlineUsers();
      fetchOfflineUsers();
    } catch (error) {
  ...
  const makeUserOnline = async (username) => {
    try {
      await api.get(`/api/municipalities/${municipalityId}/online/${username}`);
      fetchOnlineUsers();
      fetchOfflineUsers();
    } catch (error) {
        ...
        <table className="liv-user-table">
          ...
          <tbody>
            {offlineUsers.length > 0 ? (
              ...
              <tr>
                <td colSpan={canSeeAction ? 4 : 3}>{t("liveUsers.noOffline")}</td>
              </tr>
            )}
          </tbody>
        </table>
        ...
        <table className="liv-user-table">
          ...
          <tbody>
            {onlineUsers.length > 0 ? (
              ...
              <tr>
                <td colSpan={canSeeAction ? 4 : 3}>{t("liveUsers.noOnline")}</td>
              </tr>
            )}
          </tbody>
        </table>
